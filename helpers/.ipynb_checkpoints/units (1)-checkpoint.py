...
# --- kip-foot system --------------------------------------------------------
# Parallel reconstruction that renders lengths in ft and moments in kip*ft.
# Selected via setup_formatting(system="kip-ft"); the kip-inch path above is
# untouched, so existing notebooks keep their kip-inch rendering.
_FT_M = float((1 * ft).value)  # SI magnitude of 1 ft


def _unit_latex_ft(a, b):
    """LaTeX for force^a * length^b in a kip-ft system."""
    if abs(a - 1) < 1e-9 and abs(b + 2) < 1e-9:  # stress -> ksf
        return r"\mathrm{ksf}"
    num, den = [], []
...
def customary_ft(physical):
    """(magnitude_in_kip_ft, unit_latex) for a `force^a * length^b` Physical,
    or None if it isn't a pure force/length combination."""
    try:
        dims = tuple(physical.dimensions)
    except Exception:
        return None
    if len(dims) < 3:
        return None
    kg, m, s = dims[0], dims[1], dims[2]
    if any(abs(d) > 1e-9 for d in dims[3:]):
        return None
    a = kg
    if abs(s - (-2.0 * a)) > 1e-9:
        return None
    b = m - a
    try:
        magnitude = float(physical.value) / ((_KIP_N ** a) * (_FT_M ** b))
    except Exception:
        return None
    return magnitude, _unit_latex_ft(a, b)


def mag_ft(physical):
    """kip-ft magnitude of a Physical as a float, for text/tables."""
    c = customary_ft(physical)
    return c[0] if c is not None else float(physical)
...
__all__ = [
    ...
    "customary", "mag", "customary_ft", "mag_ft",
]
